...
export default function DatabaseRisetDosen() {
  const [tab, setTab] = useState<"riset" | "dosen">("riset");
  ...
  const [dosenModal, setDosenModal] = useState<"add" | "edit" | null>(null);
  const [editDosen, setEditDosen] = useState<Lecturer | null>(null);
  ...
  const [savingDosen, setSavingDosen] = useState(false);
  ...
  const filteredDosen = lecturers.filter(d => {
    const q = search.toLowerCase();
    return !q || d.name.toLowerCase().includes(q) || d.nip.includes(q) || d.departemen.toLowerCase().includes(q);
  });
  ...
  const dosenRiset = (lecturerId: string) => {
    return research
      .map((item: any) => {
        const projectMembers = members[item.id] || [];
        if (item.supervisor_id === lecturerId) {
          return { ...item, peran: "Ketua" };
        }
        const match = projectMembers.find((member: any) => member.user_id === lecturerId && member.member_type === "Dosen");
        return match ? { ...item, peran: match.peran || "Anggota" } : null;
      })
  ...
  const handleSaveDosen = async () => {
    if (!dosenForm.name.trim() || !dosenForm.nip.trim()) {
      setError("Nama dan NIP wajib diisi.");
      return;
    }

    if (dosenModal === "add" && !dosenForm.password.trim()) {
    ...
    try {
      setSavingDosen(true);
      const payload = {
        id: dosenForm.id || `D${Date.now()}`,
        name: dosenForm.name.trim(),
        initials: (dosenForm.name.trim().split(" ").map((part) => part[0]).join("").slice(0, 2) || "DS").toUpperCase(),
        nip: dosenForm.nip.trim(),
        password: dosenForm.password.trim() || null,
        email: dosenForm.email.trim() || null,
        jabatan: dosenForm.jabatan.trim() || null,
        departemen: dosenForm.departemen.trim() || null,
        keahlian: dosenForm.keahlian.split(",").map((item) => item.trim()).filter(Boolean),
        status: dosenForm.status || "Aktif"
      };
      ...
      const latestLecturers = await apiGet<Lecturer[]>("/lecturers");
      setLecturers((latestLecturers || []).map((item: any) => ({
        ...item,
        initials: item.initials || item.name?.split(" ").map((part: string) => part[0]).join("").slice(0, 2).toUpperCase() || "DS",
        color: item.color || "bg-indigo-600 text-white",
        jabatan: item.jabatan || "Dosen",
      ...
      setDosenModal(null);
      setEditDosen(null);
      setDosenForm({ id: "", name: "", nip: "", password: "", email: "", jabatan: "", departemen: "", keahlian: "", status: "Aktif" });
    } catch (err: any) {
      setError(err?.message || "Gagal menyimpan dosen.");
    } finally {
      setSavingDosen(false);
    }
  };

  if (loading) return <OperatorLayout title="Database Riset & Dosen"><div className="p-8 text-center text-muted-foreground">Memuat data...</div></OperatorLayout>;
  if (error) return <OperatorLayout title="Database Riset & Dosen"><div className="p-8 text-red-600">Error: {error}</div></OperatorLayout>;

  return (
    <OperatorLayout title="Database Riset & Dosen">
      <div className="flex flex-col gap-5 pb-4">

        {/* Tab switcher */}
        <div className="flex items-center gap-3 flex-wrap justify-between">
          <div className="flex items-center gap-1 bg-white border border-border rounded-[12px] p-1 shadow-sm">
            <button onClick={() => { setTab("riset"); setSearch(""); setSelected(null); }} className={`flex items-center gap-2 h-8 px-4 rounded-[8px] text-sm font-black transition-all ${tab === "riset" ? "bg-[#0AB600] text-white" : "text-muted-foreground hover:text-foreground"}`}>
              <FlaskConical size={15} /> Database Riset
            </button>
            <button onClick={() => { setTab("dosen"); setSearch(""); setSelectedDosen(null); }} className={`flex items-center gap-2 h-8 px-4 rounded-[8px] text-sm font-black transition-all ${tab === "dosen" ? "bg-[#0AB600] text-white" : "text-muted-foreground hover:text-foreground"}`}>
              <Users size={15} /> Database Dosen
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 px-3 py-2 bg-white border border-border rounded-[12px] w-56 focus-within:ring-2 focus-within:ring-[#0AB600]/30 transition-all">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={tab === "riset" ? "Cari judul, supervisor..." : "Cari nama, NIP..."} className="bg-transparent outline-none text-sm w-full placeholder:text-muted-foreground" />
            ...
            <button
              onClick={() => tab === "riset" ? (setModalOpen(true), setStep(0)) : (setEditDosen(null), setDosenForm({ id: "", name: "", nip: "", password: "", email: "", jabatan: "", departemen: "", keahlian: "", status: "Aktif" }), setDosenModal("add"))}
              ...
              <Plus size={15} strokeWidth={3} /> {tab === "riset" ? "Tambah Riset" : "Tambah Dosen"}
            </button>
          </div>
        </div>
                      ...
                      <div key={r.id} onClick={() => { setSelected(selected?.id === r.id ? null : r); setDetailTab("info"); }}
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex flex-wrap gap-1 mb-2">
                            {dosen.map((d: any) => (
                              <span key={d.user_id} className={`text-[10px] font-black px-2 py-0.5 rounded ${d.peran === "Ketua" ? "bg-[#E6FFE6] text-[#0AB600]" : "bg-slate-100 text-slate-600"}`}>
                                {d.initials || d.name?.charAt(0)?.toUpperCase()} {d.peran === "Ketua" ? "(Ketua)" : "(Anggota)"}
                              </span>
                            ))}
                          </div>
                      ...
                      {filteredRiset.map(r => {
                        ...
                        return (
                          <tr key={r.id} onClick={() => { setSelected(selected?.id === r.id ? null : r); setDetailTab("info"); }} className={`cursor-pointer hover:bg-slate-50 transition-colors ${selected?.id === r.id ? "bg-green-50/30" : ""}`}>
                            ...
                            <td className="px-5 py-3.5 text-center"><button className="w-7 h-7 rounded-[8px] flex items-center justify-center text-muted-foreground hover:bg-green-50 hover:text-[#0AB600] transition-colors"><Pencil size={13} /></button></td>
                          </tr>
                        );
                      })}
                    </tbody>
                  ...
                  {detailTab === "info" && (
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="pt-3 border-t border-border">
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1"><Target size={10} /> Milestone</p>
                        {((selected as any).milestones || []).map((m: any, i: number) => (
                          <div key={i} className="flex items-center gap-2 py-1.5">
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${m.done ? "bg-[#0AB600] border-[#0AB600]" : "border-slate-200 bg-white"}`}>
                              {m.done && <svg viewBox="0 0 10 10" className="w-2 h-2"><path d="M1 5l3 3 5-4.5" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round" /></svg>}
                            </div>
                            <span className={`text-xs font-bold flex-1 ${m.done ? "text-muted-foreground line-through" : "text-foreground"}`}>{m.label}</span>
          ...
          <div className="flex gap-5 items-start">
            <div className="flex-1 bg-white border border-border rounded-[14px] shadow-sm overflow-hidden">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border">
                  {filteredDosen.map(d => {
                    const risets = dosenRiset(d.id);
                    const ketua = risets.filter(r => r?.peran === "Ketua").length;
                    return (
                      <tr key={d.id} onClick={() => setSelectedDosen(selectedDosen?.id === d.id ? null : d)} className={`cursor-pointer hover:bg-slate-50 transition-colors ${selectedDosen?.id === d.id ? "bg-green-50/20" : ""}`}>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-black shrink-0 ${d.color || "bg-indigo-600 text-white"}`}>{d.initials}</div>
                            <div><p className="font-black text-foreground text-sm">{d.name}</p><p className="text-[11px] text-muted-foreground">{d.jabatan}</p></div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-3.5"><div className="flex flex-wrap gap-1">{(d.keahlian || []).slice(0, 2).map(k => <span key={k} className="text-[9px] font-black px-1.5 py-0.5 rounded bg-slate-100 text-slate-600">{k}</span>)}{(d.keahlian || []).length > 2 && <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-slate-100 text-slate-500">+{(d.keahlian || []).length - 2}</span>}</div></td>
                        <td className="px-5 py-3.5"><span className="inline-flex items-center gap-1 text-xs font-black px-2 py-0.5 rounded-full bg-green-100 text-[#0AB600]"><FlaskConical size={10} /> {ketua}</span></td>
                        <td className="px-5 py-3.5 text-xs font-bold text-muted-foreground">{risets.length - ketua}</td>
                        <td className="px-5 py-3.5"><span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${d.status === "Aktif" ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-500 border-slate-200"}`}>{d.status}</span></td>
                        <td className="px-5 py-3.5"><button onClick={e => { e.stopPropagation(); setEditDosen(d); setDosenForm({ id: d.id, name: d.name, nip: d.nip, password: "", email: d.email || "", jabatan: d.jabatan || "", departemen: d.departemen || "", keahlian: (d.keahlian || []).join(", "), status: d.status || "Aktif" }); setDosenModal("edit"); }} className="w-7 h-7 rounded-[8px] flex items-center justify-center text-muted-foreground hover:bg-green-50 hover:text-[#0AB600] transition-colors"><Pencil size={13} /></button></td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
              <div className="px-5 py-3 border-t border-border bg-slate-50/50"><p className="text-xs font-medium text-muted-foreground">{filteredDosen.length} dosen ditampilkan</p></div>
            </div>

            {/* Dosen Detail Panel */}
            {selectedDosen && (
              <div className="w-[280px] shrink-0 bg-white border border-border rounded-[14px] shadow-sm overflow-hidden max-h-[70vh] overflow-y-auto">
                <div className="px-5 py-4 border-b border-border bg-gradient-to-r from-green-50 to-white flex items-center justify-between">
                  <h3 className="text-sm font-black text-foreground">Profil Dosen</h3>
                  <button onClick={() => setSelectedDosen(null)} className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-slate-100 text-muted-foreground"><X size={14} /></button>
                </div>
                <div className="p-5">
                  <div className="flex flex-col items-center gap-2 mb-4 pb-4 border-b border-border">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center text-lg font-black ${selectedDosen.color || "bg-indigo-600 text-white"}`}>{selectedDosen.initials}</div>
                    <div className="text-center">
                      <p className="font-black text-foreground">{selectedDosen.name}</p>
                      <p className="text-xs text-muted-foreground">{selectedDosen.jabatan}</p>
                    </div>
                  </div>
                  {[["NIP", selectedDosen.nip], ["Email", selectedDosen.email], ["Departemen", selectedDosen.departemen]].map(([l, v]) => (
                    <div key={l} className="flex gap-2 text-xs mb-2"><span className="font-black text-muted-foreground w-20">{l}</span><span className="font-bold text-foreground">{v}</span></div>
                  ))}
                  <div className="mt-3 mb-3"><p className="text-[10px] font-black text-muted-foreground uppercase tracking-wide mb-1.5">Keahlian</p><div className="flex flex-wrap gap-1">{(selectedDosen.keahlian || []).map(k => <span key={k} className="text-[10px] font-black px-2 py-0.5 rounded-md bg-blue-50 text-blue-700">{k}</span>)}</div></div>
                  <div className="border-t border-border pt-3">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wide mb-2">Keanggotaan Riset</p>
                    {dosenRiset(selectedDosen.id).map((r, i) => r && (
                      <div key={i} className="flex items-center gap-2 py-1.5">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${r.peran === "Ketua" ? "bg-[#0AB600]" : "bg-slate-300"}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-black text-foreground line-clamp-1">{(r as any).short_title || (r as any).title}</p>
                          <p className="text-[9px] font-bold text-muted-foreground">{r.peran}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {/* Dosen Modal */}
      {dosenModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setDosenModal(null)}>
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[500px]" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-border flex items-center justify-between">
              <h3 className="font-black text-foreground">{dosenModal === "add" ? "Tambah Dosen" : "Edit Dosen"}</h3>
              <button onClick={() => setDosenModal(null)} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 text-muted-foreground"><X size={16} /></button>
            </div>
            <div className="p-6 grid grid-cols-2 gap-4">
              {[
                { key: "name", label: "Nama Lengkap", ph: "Dr. / Prof. ...", val: dosenForm.name, col: "col-span-2" },
                { key: "nip", label: "NIP", ph: "Nomor Induk Pegawai", val: dosenForm.nip, col: "" },
                { key: "password", label: dosenModal === "add" ? "Password" : "Password Baru (Opsional)", ph: dosenModal === "add" ? "Minimal 6 karakter" : "Kosongkan jika tidak diubah", val: dosenForm.password, col: "" },
                { key: "email", label: "Email", ph: "nama@ac.id", val: dosenForm.email, col: "" },
                { key: "jabatan", label: "Jabatan", ph: "Lektor, Guru Besar...", val: dosenForm.jabatan, col: "" },
                { key: "departemen", label: "Departemen", ph: "Teknik Informatika", val: dosenForm.departemen, col: "" },
                { key: "keahlian", label: "Keahlian", ph: "ML, IoT, Cloud (pisah koma)", val: dosenForm.keahlian, col: "col-span-2" },
              ].map(f => (
                <div key={f.label} className={f.col}>
                  <label className="text-xs font-black text-foreground block mb-1.5">{f.label}</label>
                  <input type={f.key === "password" ? "password" : "text"} value={f.val} onChange={e => setDosenForm((prev) => ({ ...prev, [f.key]: e.target.value }))} placeholder={f.ph} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" />
                </div>
              ))}
              <div className="col-span-2">
                <label className="text-xs font-black text-foreground block mb-1.5">Status</label>
                <select value={dosenForm.status} onChange={e => setDosenForm((prev) => ({ ...prev, status: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none cursor-pointer">
                  <option>Aktif</option>
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setDosenModal(null)} className="flex-1 h-10 border border-border rounded-[10px] text-sm font-bold text-muted-foreground hover:bg-slate-50">Batal</button>
              <button disabled={savingDosen} onClick={handleSaveDosen} className="flex-1 h-10 bg-[#0AB600] hover:bg-[#099800] disabled:bg-[#8ad98a] text-white text-sm font-black rounded-[10px]">{savingDosen ? "Menyimpan..." : "Simpan"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[560px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-border flex items-center justify-between">
              <div><h3 className="font-black text-foreground">Tambah Riset Baru</h3><p className="text-xs text-muted-foreground mt-0.5">Langkah {step + 1} dari {STEP_LABELS.length}: {STEP_LABELS[step]}</p></div>
            ...
            <div className="px-6 pt-4 flex items-center gap-2">
              {STEP_LABELS.map((l, i) => (
                <div key={i} className="contents">
                  <div onClick={() => i < step && setStep(i)} className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-black cursor-pointer ${i === step ? "bg-[#0AB600] text-white" : i < step ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>{i + 1}</div>
                  {i < STEP_LABELS.length - 1 && <div className={`flex-1 h-0.5 ${i < step ? "bg-[#0AB600]/50" : "bg-slate-100"}`} />}
                </div>
              ))}
            </div>
            <div className="p-6 grid grid-cols-2 gap-4">
              {step === 0 && <>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Judul Riset</label><input value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} placeholder="Judul lengkap penelitian" className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Deskripsi</label><textarea value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} rows={3} placeholder="Ringkasan penelitian..." className="w-full px-3 py-2 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all resize-none" /></div>
                <div><label className="text-xs font-black text-foreground block mb-1.5">Kategori</label><input value={formData.category} onChange={(e) => setFormData(prev => ({ ...prev, category: e.target.value }))} placeholder="IoT, AI, Blockchain..." className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
              ...
              {step === 1 && <>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Ketua Dosen</label><select value={formData.supervisorId} onChange={(e) => setFormData(prev => ({ ...prev, supervisorId: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none cursor-pointer"><option value="">-- Pilih Dosen --</option>{dosenData.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
              ...
              {step === 2 && <>
                <div><label className="text-xs font-black text-foreground block mb-1.5">Tanggal Mulai</label><input type="date" value={formData.startDate} onChange={(e) => setFormData(prev => ({ ...prev, startDate: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
                <div><label className="text-xs font-black text-foreground block mb-1.5">Tanggal Selesai</label><input type="date" value={formData.endDate} onChange={(e) => setFormData(prev => ({ ...prev, endDate: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Mitra</label><input value={formData.mitra} onChange={(e) => setFormData(prev => ({ ...prev, mitra: e.target.value }))} placeholder="Nama institusi mitra" className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Sumber Pendanaan</label><input value={formData.funding} onChange={(e) => setFormData(prev => ({ ...prev, funding: e.target.value }))} placeholder="DIKTI, Industri, dll." className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-[#0AB600]/30 transition-all" /></div>
              </>}
              {step === 3 && <>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Milestone</label>
                  <div className="flex flex-col gap-2">
                    ...
                    <button onClick={() => setFormData(prev => ({ ...prev, milestones: [...prev.milestones, `Milestone ${prev.milestones.length + 1}`] }))} className="h-9 border-2 border-dashed border-green-200 rounded-[10px] text-xs font-bold text-[#0AB600] hover:bg-green-50 transition-colors">+ Tambah Milestone</button>
                  </div>
                </div>
              </>}
            </div>
