...
export default function LayananSurat() {
  ...
  const handleProses = (id: string) => {
    setProcessingId(id);
    setTimeout(() => {
      apiPatch<{ message: string }>(`/letter-requests/${id}/status`, { status: "Diproses" })
        .then(() => {
          setLetters((prev) => prev.map((letter) => letter.id === id ? { ...letter, status: "Diproses" as const } : letter));
          setDetail((prev) => prev?.id === id ? { ...prev, status: "Diproses" } : prev);
          showToast("Permintaan surat berhasil diproses.");
        })
        .catch((err) => setError(err?.message || "Gagal memproses surat."))
  ...
  const handleDelete = async (item: LetterRequestAll) => {
    ...
    setError("");
    try {
      await apiDelete(`/letter-requests/${item.id}`);
      setLetters((prev) => prev.filter((letter) => letter.id !== item.id));
      setDetail((prev) => prev?.id === item.id ? null : prev);
      showToast("Pengajuan surat berhasil dihapus.");
    } catch (err: any) {
      setError(err?.message || "Gagal menghapus pengajuan surat.");
    } finally {
      setDeletingId(null);
    }
  };
                  ...
                  <tr key={l.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1">
                        ...
                        {l.status === "Siap Unduh" && (
                          <a href={resolveApiAssetUrl(l.fileUrl) || "#"} target="_blank" rel="noreferrer" className={`h-7 px-2 rounded-[8px] text-[10px] font-black border transition-colors flex items-center gap-1 ${resolveApiAssetUrl(l.fileUrl) ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" : "bg-slate-50 text-slate-400 border-slate-200 pointer-events-none"}`}>
                            <Download size={11} /> Unduh
                          </a>
                        )}
                        <button onClick={() => handleDelete(l)} disabled={deletingId === l.id} className="h-7 px-2 rounded-[8px] text-[10px] font-black bg-slate-50 hover:bg-red-50 text-slate-500 hover:text-red-600 border border-slate-200 transition-colors disabled:opacity-50 flex items-center gap-1">
                          <Trash2 size={11} /> {deletingId === l.id ? "Menghapus..." : "Hapus"}
                        </button>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="h-full w-full max-w-[400px] bg-white shadow-2xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 text-xs border-t border-border pt-4">
                {[["Jenis Surat", detail.jenis], ["Tanggal", detail.tanggal], ["Nomor Surat", detail.nomorSurat || "-"], ["Estimasi", detail.estimasi || "-"]].map(([label, value]) => (
                  <div key={String(label)} className="flex gap-2"><span className="font-black text-muted-foreground w-24 shrink-0">{label}</span><span className="font-bold text-foreground">{value}</span></div>
                ))}
              </div>
              <div>
              ...
              {detail.status === "Siap Unduh" && (
                <a href={resolveApiAssetUrl(detail.fileUrl) || "#"} target="_blank" rel="noreferrer" className={`flex items-center justify-center gap-2 h-10 font-black text-sm rounded-[10px] transition-colors ${resolveApiAssetUrl(detail.fileUrl) ? "bg-emerald-500 hover:bg-emerald-600 text-white" : "bg-slate-100 text-slate-400 pointer-events-none"}`}>
          ...
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[440px]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <label className="border-2 border-dashed border-amber-200 rounded-[12px] p-8 text-center hover:bg-amber-50 transition-colors cursor-pointer block" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); handlePickedFile(event.dataTransfer.files?.[0]); }}>
                ...
                <input type="file" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" className="hidden" onChange={(event) => handlePickedFile(event.target.files?.[0])} />
              </label>
            </div>
