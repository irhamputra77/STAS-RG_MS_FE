...
import { Search, Plus, X, ChevronRight, Users, Calendar, Building, Target, Pencil, LayoutGrid, List, Shield, Trash2, Check } from "lucide-react";
...
export default function DatabaseRiset() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [researchData, studentData, lecturerData] = await Promise.all([
          apiGet<ResearchProject[]>("/research"),
          apiGet<Array<any>>("/students"),
          apiGet<Array<any>>("/lecturers")
        ]);
        setResearch(researchData || []);
        setStudents((studentData || []).map((item: any) => ({ id: item.id, name: item.name, initials: item.initials })));
        setLecturers((lecturerData || []).map((item: any) => ({ id: item.id, name: item.name })));
      } catch (err: any) {
      ...
      const loadMembersAndAccess = async () => {
        try {
          const [memberData, accessData] = await Promise.all([
            apiGet(`/research/${selected.id}/members`),
            apiGet(`/research/${selected.id}/board-access`)
          ]);
  ...
  const filtered = research.filter(r => {
    ...
    const mq = !q || r.title.toLowerCase().includes(q) || r.supervisor_name?.toLowerCase().includes(q);
    const ms = filterStatus === "Semua" || r.status === filterStatus;
    return mq && ms;
  });

  const statusColor = (s: string) => s === "Aktif" ? "bg-emerald-100 text-emerald-700 border-emerald-200" : s === "Selesai" ? "bg-slate-100 text-slate-600 border-slate-200" : "bg-red-100 text-red-600 border-red-200";

  const revokeAccess = async (risetId: string, memberId: string) => {
    await apiDelete(`/research/${risetId}/board-access/${memberId}`);
    setBoardAccess(prev => ({ ...prev, [risetId]: (prev[risetId] || []).filter(x => x !== memberId) }));
  };

  const grantAccess = async (risetId: string, memberId: string) => {
    await apiPost(`/research/${risetId}/board-access`, { userId: memberId });
    setBoardAccess(prev => ({ ...prev, [risetId]: [...(prev[risetId] || []), memberId] }));
  };
  ...
  const addablePeople = [
    ...lecturers.map((item) => ({ user_id: item.id, name: item.name, initials: item.name?.charAt(0), member_type: "Dosen" })),
  ...
  const handleAddMembers = async () => {
    ...
    try {
      ...
      for (const userId of selectedNewMembers) {
        const candidate = addablePeople.find((item) => item.user_id === userId);
      ...
      const memberData = await apiGet(`/research/${selected.id}/members`);
      setMembers((prev) => ({ ...prev, [selected.id]: memberData || [] }));
  ...
  const handleCreateResearch = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        short_title: formData.title.trim(),
      ...
      <div className="flex flex-col gap-5 pb-4">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap flex-1">
            ...
            <div className="flex items-center bg-white border border-border rounded-[10px] overflow-hidden">
              {([["grid", <LayoutGrid size={15} />], ["table", <List size={15} />]] as [string, React.ReactNode][]).map(([v, icon]) => (
                <button key={v} onClick={() => setView(v as "grid" | "table")} className={`h-9 w-9 flex items-center justify-center transition-colors ${view === v ? "bg-amber-500 text-white" : "text-muted-foreground hover:bg-slate-50"}`}>{icon}</button>
              ))}
            </div>
          </div>
          <button onClick={() => { setModalOpen(true); setStep(0); }} className="flex items-center gap-2 h-9 px-4 bg-amber-500 hover:bg-amber-600 text-white text-sm font-black rounded-[10px] shadow-sm transition-colors">
        ...
        <div className="flex gap-5 items-start">
          ...
          <div className="flex-1 min-w-0">
            {view === "grid" ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {filtered.map(r => {
                  const rMembers = members[r.id] || [];
                  return (
                    <div key={r.id} className={`bg-white border rounded-[14px] shadow-sm overflow-hidden hover:shadow-md transition-all cursor-pointer ${selected?.id === r.id ? "border-amber-400" : "border-border"}`} onClick={() => { setSelected(selected?.id === r.id ? null : r); setDetailTab("info"); }}>
                      <div className="h-20 bg-gradient-to-br from-[#6C47FF] to-[#1A1A2E] relative overflow-hidden p-4 flex items-end">
                        ...
                        <div className="relative z-10"><span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${statusColor(r.status)} bg-white`}>{r.status}</span></div>
                      </div>
                      <div className="p-4">
                        <h3 className="font-black text-foreground text-sm leading-snug mb-1 line-clamp-2">{r.title}</h3>
                        <p className="text-xs text-muted-foreground mb-3">{r.supervisor_name} · {r.category}</p>
                        <div className="flex items-center justify-between">
                          <div className="flex -space-x-2">
                            {rMembers.slice(0, 4).map((m: any) => <div key={m.id} className="w-7 h-7 rounded-full border-2 border-white bg-indigo-600 text-white text-[10px] font-black flex items-center justify-center">{m.initials || m.name?.charAt(0).toUpperCase()}</div>)}
                            {rMembers.length > 4 && <div className="w-7 h-7 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-500">+{rMembers.length - 4}</div>}
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1.5 bg-slate-100 rounded-full"><div className="bg-[#6C47FF] h-1.5 rounded-full" style={{ width: `${r.progress || 0}%` }} /></div>
                            <span className="text-[10px] font-black text-[#6C47FF]">{r.progress || 0}%</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white border border-border rounded-[14px] shadow-sm overflow-hidden">
                <table className="w-full text-sm text-left">
                  <thead><tr className="bg-slate-50 border-b border-border">
                    {["Judul Riset", "Supervisor", "Periode", "Anggota", "Progress", "Status", ""].map(h => <th key={h} className="px-5 py-3 text-xs font-black text-muted-foreground uppercase tracking-wide">{h}</th>)}
                  </tr></thead>
                  <tbody className="divide-y divide-border">
                    {filtered.map(r => (
                      <tr key={r.id} onClick={() => { setSelected(selected?.id === r.id ? null : r); setDetailTab("info"); }} className={`cursor-pointer hover:bg-slate-50 transition-colors ${selected?.id === r.id ? "bg-amber-50/40" : ""}`}>
                        <td className="px-5 py-3.5 font-black text-foreground max-w-[220px]"><p className="line-clamp-1">{r.title}</p></td>
                        <td className="px-5 py-3.5 text-muted-foreground text-xs">{r.supervisor_name}</td>
                        <td className="px-5 py-3.5 text-muted-foreground text-xs whitespace-nowrap">{r.period_text}</td>
                        <td className="px-5 py-3.5 text-foreground text-xs">{(members[r.id] || []).length} org</td>
                        <td className="px-5 py-3.5"><div className="flex items-center gap-2"><div className="w-20 h-1.5 bg-slate-100 rounded-full"><div className="bg-[#6C47FF] h-1.5 rounded-full" style={{ width: `${r.progress || 0}%` }} /></div><span className="text-[10px] font-black text-[#6C47FF]">{r.progress || 0}%</span></div></td>
                        ...
                        <td className="px-5 py-3.5"><button className="w-7 h-7 rounded-[8px] flex items-center justify-center text-muted-foreground hover:bg-amber-50 hover:text-amber-600 transition-colors"><ChevronRight size={14} /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* Detail Panel */}
          {selected && (
            <div className="w-[340px] shrink-0 bg-white border border-border rounded-[14px] shadow-sm overflow-hidden max-h-[78vh] overflow-y-auto">
              <div className="h-16 bg-gradient-to-br from-[#6C47FF] to-[#1A1A2E] relative">
                ...
                <div className="absolute bottom-0 left-0 right-0 px-4 pb-0 flex gap-1">
                  {(["info", "anggota", "akses"] as const).map(t => (
                    <button key={t} onClick={() => setDetailTab(t)} className={`px-3 py-2 text-[11px] font-black transition-colors capitalize rounded-t-[8px] ${detailTab === t ? "bg-white text-foreground" : "text-white/70 hover:text-white"}`}>
                      {t === "info" ? "Info" : t === "anggota" ? "Anggota" : "Board Access"}
                    </button>
                  ))}
                </div>
              ...
              <div className="p-5">
                {detailTab === "info" && (
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button className="flex items-center gap-1 text-[10px] font-black text-amber-600 hover:bg-amber-50 px-2 py-1 rounded-[8px] transition-colors"><Pencil size={11} /> Edit</button>
                    </div>
                    ...
                    {[["Supervisor", selected.supervisor_name], ["Periode", selected.period_text], ["Mitra", selected.mitra], ["Pendanaan", selected.funding], ["Kategori", selected.category]].map(([l, v]) => (
                      <div key={l} className="flex gap-2 text-xs"><span className="font-black text-muted-foreground w-20 shrink-0">{l}</span><span className="font-bold text-foreground">{v}</span></div>
                    ))}
                  </div>
                )}
                ...
                {detailTab === "anggota" && (
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-black text-foreground flex items-center gap-1"><Users size={12} /> Anggota Tim ({(members[selected.id] || []).length})</p>
                      <button onClick={() => { setSelectedNewMembers([]); setAddMemberModal(true); }} className="text-[10px] font-black text-amber-600 hover:bg-amber-50 px-2 py-1 rounded-[8px] transition-colors flex items-center gap-1"><Plus size={10} strokeWidth={3} /> Tambah</button>
                    </div>
                    {(members[selected.id] || []).map((m: any) => (
                      <div key={m.id} className="flex items-center gap-2 py-2 border-b border-border last:border-0">
                        <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black shrink-0">{m.initials || m.name?.charAt(0).toUpperCase()}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-black text-foreground truncate">{m.name}</p>
                          <div className="flex items-center gap-1">
                            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${m.member_type === "Dosen" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}>{m.member_type}</span>
                          </div>
                        </div>
                        <button className="w-6 h-6 rounded-full flex items-center justify-center text-muted-foreground hover:bg-red-50 hover:text-red-500 transition-colors"><Trash2 size={11} /></button>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {detailTab === "akses" && (
                  <div>
                    <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-[10px] text-xs text-amber-700">
                      <p className="font-black mb-0.5">Kontrol Akses Board</p>
                    ...
                    {[{ label: "Semua Operator", sub: "Full access", initials: "OP", color: "bg-amber-500 text-white" }, { label: selected.supervisor_name, sub: "Ketua Riset", initials: selected.supervisor_name?.charAt(0).toUpperCase(), color: "bg-indigo-600 text-white" }].map(item => (
                    ...
                    <div className="flex items-center justify-between mt-4 mb-2">
                      ...
                      {nonAccessMembers.length > 0 && (
                        <button onClick={() => setAddMemberModal(true)} className="text-[10px] font-black text-amber-600 hover:bg-amber-50 px-2 py-0.5 rounded-[8px] transition-colors flex items-center gap-0.5"><Plus size={9} strokeWidth={3} /> Beri</button>
                      )}
                    </div>
                    {currentAccess.length === 0 ? (
                      <p className="text-xs text-muted-foreground italic py-2">Belum ada mahasiswa dengan akses edit.</p>
                    ) : currentAccess.map(mid => {
                      const m = mahasiswaInRiset.find((x: any) => x.user_id === mid);
                      if (!m) return null;
                      return (
                        <div key={mid} className="flex items-center gap-2 py-2 bg-blue-50 border border-blue-100 rounded-[10px] px-3 mb-1.5">
                          <div className="w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center text-[9px] font-black shrink-0">{m.initials || m.name?.charAt(0).toUpperCase()}</div>
                    ...
                    {nonAccessMembers.map((m: any) => (
                      <div key={m.user_id} className="flex items-center gap-2 py-2">
                        <div className="w-7 h-7 rounded-full bg-slate-300 text-white flex items-center justify-center text-[9px] font-black shrink-0">{m.initials || m.name?.charAt(0).toUpperCase()}</div>
                        ...
                        <button onClick={() => grantAccess(selected.id, m.user_id)} className="w-6 h-6 rounded-full bg-amber-100 hover:bg-amber-500 text-amber-700 hover:text-white flex items-center justify-center transition-all"><Plus size={10} strokeWidth={3} /></button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setAddMemberModal(false)}>
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[480px]" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-5 border-b border-border flex items-center justify-between">
              <h3 className="font-black text-foreground">Tambah Anggota – {selected.short_title || selected.title}</h3>
            ...
            <div className="p-6">
              <div className="flex flex-col gap-2 max-h-[250px] overflow-y-auto">
                {addablePeople.slice(0, 12).map((p: any) => (
                  <label key={p.user_id} className="flex items-center gap-3 p-3 rounded-[10px] border border-border hover:bg-slate-50 cursor-pointer transition-colors">
                    <input type="checkbox" checked={selectedNewMembers.includes(p.user_id)} onChange={() => toggleNewMember(p.user_id)} className="accent-amber-500 shrink-0" />
                    ...
                    <div className="flex-1">
                      ...
                      <span className={`text-[10px] font-black px-1.5 py-0.5 rounded ${p.member_type === "Dosen" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}>{p.member_type}</span>
                    </div>
                  </label>
                ))}
              </div>
              <div className="mt-4">
                ...
                <select value={selectedPeran} onChange={e => setSelectedPeran(e.target.value)} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none cursor-pointer">
                  {PERAN_OPTIONS.map(p => <option key={p}>{p}</option>)}
                </select>
              </div>
            </div>
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => setAddMemberModal(false)} className="flex-1 h-10 border border-border rounded-[10px] text-sm font-bold text-muted-foreground hover:bg-slate-50 transition-colors">Batal</button>
              <button disabled={savingMembers || selectedNewMembers.length === 0} onClick={handleAddMembers} className="flex-1 h-10 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white text-sm font-black rounded-[10px] transition-colors">{savingMembers ? "Menyimpan..." : "Tambahkan"}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[560px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 grid grid-cols-2 gap-4">
              ...
              {step === 1 && <>
                ...
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Anggota Mahasiswa</label><div className="flex flex-col gap-1.5">{students.map(s => <label key={s.id} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-foreground"><input type="checkbox" checked={formData.studentIds.includes(s.id)} onChange={(e) => { if (e.target.checked) { setFormData(prev => ({ ...prev, studentIds: [...prev.studentIds, s.id] })); } else { setFormData(prev => ({ ...prev, studentIds: prev.studentIds.filter(id => id !== s.id) })); } }} className="accent-amber-500" />{s.name}</label>)}</div></div>
              </>}
              {step === 2 && <>
                <div><label className="text-xs font-black text-foreground block mb-1.5">Tanggal Mulai</label><input type="date" value={formData.startDate} onChange={(e) => setFormData(prev => ({ ...prev, startDate: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all" /></div>
                <div><label className="text-xs font-black text-foreground block mb-1.5">Tanggal Selesai</label><input type="date" value={formData.endDate} onChange={(e) => setFormData(prev => ({ ...prev, endDate: e.target.value }))} className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all" /></div>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Mitra</label><input value={formData.mitra} onChange={(e) => setFormData(prev => ({ ...prev, mitra: e.target.value }))} placeholder="Nama institusi mitra" className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all" /></div>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Sumber Pendanaan</label><input value={formData.funding} onChange={(e) => setFormData(prev => ({ ...prev, funding: e.target.value }))} placeholder="DIKTI, Industri, dll." className="w-full h-10 px-3 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all" /></div>
              </>}
              {step === 3 && <>
                <div className="col-span-2"><label className="text-xs font-black text-foreground block mb-1.5">Milestone</label>
                  <div className="flex flex-col gap-2">
                    ...
                    <button onClick={() => setFormData(prev => ({ ...prev, milestones: [...prev.milestones, `Milestone ${prev.milestones.length + 1}`] }))} className="h-9 border-2 border-dashed border-amber-200 rounded-[10px] text-xs font-bold text-amber-600 hover:bg-amber-50 transition-colors">+ Tambah Milestone</button>
                  </div>
                </div>
              </>}
            </div>
            <div className="px-6 pb-6 flex gap-3">
              ...
              <button onClick={() => step < STEP_LABELS.length - 1 ? setStep(s => s + 1) : handleCreateResearch()} disabled={savingRiset} className="flex-1 h-10 bg-amber-500 hover:bg-amber-600 text-white text-sm font-black rounded-[10px] transition-colors disabled:bg-amber-400">
