...
export default function PersetujuanCuti() {
  ...
  const counts = { menunggu: leaves.filter(l => l.status === "Menunggu").length, disetujui: leaves.filter(l => l.status === "Disetujui").length, ditolak: leaves.filter(l => l.status === "Ditolak").length };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setLeaves(prev => prev.map(l => l.id === confirm.item.id ? { ...l, status: newStatus, reviewedBy: user?.name || "Admin Operator", reviewedAt: new Date().toISOString(), reviewNote: note } : l));
      setDetail(d => d?.id === confirm.item.id ? { ...d, status: newStatus } : d);
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setConfirm(null)}>
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[420px] p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-center text-sm text-muted-foreground mb-4">
              Anda akan <strong>{confirm.action === "Setujui" ? "menyetujui" : "menolak"}</strong> cuti {confirm.item.durasi} hari dari <strong>{confirm.item.mahasiswaNama}</strong>
            </p>
            <div className="mb-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleConfirm} className={`flex-1 h-10 text-white text-sm font-black rounded-[10px] transition-colors ${confirm.action === "Setujui" ? "bg-emerald-500 hover:bg-emerald-600" : "bg-red-500 hover:bg-red-600"}`}>
                Ya, {confirm.action}
              </button>
            </div>
          </div>
