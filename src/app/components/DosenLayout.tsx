...
import {
  LayoutDashboard, FlaskConical, BookOpen, Kanban, Award, FileText,
  Bell, Search, BookMarked, X, CheckCheck, LogOut,
} from "lucide-react";
...
const navItems = [
  ...
  { name: "Review Draft",        path: "/dosen/draft",      icon: FileText },
...
export function DosenLayout({ children, title = "Dashboard Dosen" }: LayoutProps) {
  ...
  return (
    <div className="h-screen w-screen bg-slate-50 overflow-hidden flex">
      <div className="w-full flex h-screen overflow-hidden bg-slate-50">
        {/* ── Sidebar ── */}
          ...
          <div className="h-[60px] flex items-center px-6 border-b border-border">
            <div className="font-black text-xl text-primary flex items-center gap-2">
              {branding.logoDataUrl ? (
                ...
                <div className="w-8 h-8 rounded-[10px] bg-primary flex items-center justify-center text-white"><BookMarked size={16} /></div>
              )}
              {branding.appName}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-[60px] bg-white border-b border-border flex items-center justify-between px-8 shrink-0">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3 px-4 py-2 bg-slate-50 border border-border rounded-[12px] w-60 focus-within:ring-2 focus-within:ring-green-300 transition-all">
              ...
              <div className="flex items-center gap-4">
                <div ref={bellRef} className="relative">
                  <button onClick={() => setPanelOpen(v => !v)} className={`relative p-1.5 rounded-[10px] transition-colors ${panelOpen ? "bg-green-100 text-primary" : "text-muted-foreground hover:bg-slate-100"}`}>
                    <Bell size={21} />
                    {unread > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 bg-red-500 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-black text-white">{unread}</span>}
                  </button>
                  {panelOpen && (
                    <div className="absolute top-[calc(100%+10px)] right-0 w-[360px] bg-white rounded-[16px] shadow-2xl border border-border z-[300]">
                      <div className="px-5 pt-4 pb-3 border-b border-border flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                          {unread > 0 && <button onClick={markAll} className="text-xs font-bold text-primary hover:bg-green-50 px-2 py-1 rounded-[8px] flex items-center gap-1"><CheckCheck size={13} /> Baca Semua</button>}
                      ...
                      <div className="max-h-[320px] overflow-y-auto divide-y divide-border/50">
                        {notifs.map(n => (
                          <div key={n.id} onClick={() => markRead(n.id)} className={`group flex gap-3 px-5 py-3.5 cursor-pointer transition-colors relative ${n.read ? "hover:bg-slate-50" : "bg-green-50/40 hover:bg-green-50"}`}>
