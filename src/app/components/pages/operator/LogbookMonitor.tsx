...
export default function LogbookMonitor() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [studentRows, logRows] = await Promise.all([
          ...
          apiGet<Array<any>>(`/logbooks?_=${cacheKey}`),
        ]);
        ...
        logRows.forEach((item: any) => {
          ...
          if (!risetByStudent[item.student_id]) {
            risetByStudent[item.student_id] = [];
          }

          if (!risetByStudent[item.student_id].includes(risetName)) {
            risetByStudent[item.student_id].push(risetName);
          }
        });
        ...
        const normalizedEntries = logRows.map((item) => {
          ...
          return {
            ...
            mahasiswaPhotoUrl: item.student_photo_url || item.photoUrl || item.photo_url || relatedStudent?.photoUrl || null,
        ...
        setSelectedStudent((current) => {
          if (current && mappedStudents.some((student) => student.id === current.id)) {
            const refreshed = mappedStudents.find((student) => student.id === current.id);
            return refreshed || current;
          }

          return mappedStudents[0] || null;
        });
      } catch (err: any) {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const mq = !q || student.name.toLowerCase().includes(q) || String(student.nim || "").toLowerCase().includes(q);
    const mr = filterRiset === "Semua" || student.riset.includes(filterRiset);

    return mq && mr;
  });

  const studentEntries = useMemo(() => {
    const byStudent = entries.filter((entry) => entry.mahasiswaId === selectedStudent?.id);

    if (!dateFrom && !dateTo) return byStudent;

    return byStudent.filter((entry) => {
      const raw = entry.rawDate || entry.fullDate;
  ...
  return (
    <OperatorLayout title="Logbook Mahasiswa">
      <div className="flex gap-5 items-start pb-4 h-full">
        ...
        <div className="w-[280px] shrink-0 bg-white border border-border rounded-[14px] shadow-sm overflow-hidden flex flex-col max-h-[calc(100vh-140px)]">
          <div className="p-4 border-b border-border">
            ...
            <select
              value={filterRiset}
              onChange={(event) => setFilterRiset(event.target.value)}
              className="w-full h-9 px-3 bg-white border border-border rounded-[10px] text-xs font-bold focus:outline-none cursor-pointer"
            >
              {risetOptions.map((option) => (
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-border">
            {filteredStudents.map((student) => {
              const entriesByStudent = entries.filter((entry) => entry.mahasiswaId === student.id);
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3.5 hover:bg-amber-50/40 transition-colors flex items-start gap-3 ${
                    isActive ? "bg-amber-50 border-l-[3px] border-amber-500" : "border-l-[3px] border-transparent"
                  }`}
                >
                  <ProfileAvatar
                    name={student.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-black ${isActive ? "text-amber-700" : "text-foreground"}`}>
                      {student.name}
                    </p>

                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {student.riset.map((riset: string) => (
                        <span key={riset} className="text-[9px] font-black px-1 py-0.5 rounded bg-slate-100 text-slate-600">
                          {riset}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <span className="text-[10px] text-muted-foreground font-medium">
                        {entriesByStudent.length} entri bulan ini
                      </span>
                      <span className={`text-[10px] font-bold ${student.status === "Aktif" ? "text-emerald-600" : "text-muted-foreground"}`}>
                        {student.status}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      {selectedStudent.riset.map((riset: string) => (
                        <span
                          key={riset}
                          className={`text-[10px] font-black px-2 py-0.5 rounded ${
                            riset === "Riset A" ? "bg-[#F8F5FF] text-[#6C47FF]" : "bg-emerald-50 text-emerald-700"
                          }`}
                        >
                          {riset}
                        </span>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2 bg-white border border-border rounded-[10px] px-3 py-2 text-xs">
                  ...
                  <input
                    ...
                    className="outline-none text-xs text-foreground bg-transparent"
                  />
                ...
                <select className="h-9 px-3 bg-white border border-border rounded-[10px] text-xs font-bold focus:outline-none cursor-pointer">
                  {["Semua Riset", ...risetOptions.filter((option) => option !== "Semua")].map((option) => (
                    <option key={option}>{option}</option>
                  ))}
                </select>
              </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span
                                className={`text-[10px] font-black px-1.5 py-0.5 rounded ${
                                  entry.riset === "Riset A" ? "bg-[#F8F5FF] text-[#6C47FF]" : "bg-emerald-50 text-emerald-700"
                                }`}
                              >
                                {entry.riset}
                              </span>
                            ...
                            <h3 className="font-black text-foreground text-sm group-hover:text-amber-700 transition-colors leading-snug">
                              {entry.title}
                            </h3>
                            ...
                            {entry.verificationStatus && (
                              <span
                                className={`inline-flex mt-1.5 text-[10px] font-black px-2 py-0.5 rounded-full ${
                                  entry.verificationStatus === "Terverifikasi"
                        ...
                        <div className="mt-1 flex shrink-0 items-center gap-1">
                          ...
                          <button
                            onClick={(event) => {
                              ...
                              void handleDeleteLogbook(entry);
                            }}
      ...
      {selectedEntry && (
        ...
        >
          <div
            className="bg-white rounded-[20px] shadow-2xl w-full max-w-[560px] max-h-[85vh] overflow-y-auto"
          ...
          >
            <div className="px-6 py-5 border-b border-border flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`text-[10px] font-black px-2 py-0.5 rounded ${
                      selectedEntry.riset === "Riset A" ? "bg-[#F8F5FF] text-[#6C47FF]" : "bg-emerald-50 text-emerald-700"
                    }`}
                  >
                    {selectedEntry.riset}
                  </span>
              ...
              <button
                onClick={() => setSelectedEntry(null)}
                className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 text-muted-foreground ml-4 shrink-0"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              {selectedEntry.verificationStatus && (
                <div className="p-3 rounded-[10px] border border-border bg-slate-50">
                  ...
                  <p className={`text-xs font-black ${selectedEntry.verificationStatus === "Terverifikasi" ? "text-emerald-700" : "text-amber-700"}`}>
                    {selectedEntry.verificationStatus}
                  </p>
            ...
            <div className="px-6 pb-5 flex gap-3">
              <button
                onClick={() => void handleDeleteLogbook(selectedEntry)}
              ...
              <button
                onClick={() => {
                  setVerifyModal({ entry: selectedEntry, status: "Perlu Revisi" });
                  setVerificationNote(selectedEntry.verificationNote || "");
                }}
                className="flex-1 h-10 bg-amber-500 hover:bg-amber-600 text-white text-sm font-black rounded-[10px] transition-colors"
              >
                Perlu Revisi
              </button>
              ...
              <button
                onClick={() => {
                  setVerifyModal({ entry: selectedEntry, status: "Terverifikasi" });
                  setVerificationNote(selectedEntry.verificationNote || "");
                }}
                className="flex-1 h-10 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-black rounded-[10px] transition-colors"
              >
                Verifikasi
              </button>
            </div>
          ...
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-[460px]" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="p-6 flex flex-col gap-3">
              <p className="text-sm text-foreground">
                Status: <span className="font-black">{verifyModal.status}</span>
              </p>
              <textarea
                ...
                className="w-full px-3 py-2 rounded-[10px] border border-border text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all resize-none"
              />
            </div>
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              <button
                ...
                className="flex-1 h-10 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white text-sm font-black rounded-[10px] transition-colors"
              >
